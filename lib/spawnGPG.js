...
function streamToString (stream) {
  ...
  }).catch((e) => console.log('error in streamToString', e));
}
...
module.exports.streaming = async function(options, args, cb) {
  ...
  sourceStream.pipe(gpg.stdin).on('error', (err)=>{
    console.log('error in stdin', err);
  });
  gpg.stdout.pipe(destStream).on('error', (err)=>{
    console.log('error in stdout', err);
  });
